...
while timestep <maxtimestep
  ...
  dt = real_dt;
   ...
   for i = 2:imax-1
      ...
      p_plus = (gamma_at_i(i)-1).*e_plus+(0.5).*rho_plus.*u_plus.^2;
      p_i = (gamma_at_i(i)-1).*e_i+(0.5).*rho_i.*u_i.^2;
      ...
      a_hat_plus = sqrt((gamma_at_i(i)-1)*(h_hat_plus-0.5*u_hat_plus.^2));      
...
%% PART 3: NOW DEAL WITH I MINUS HALF 
    ...
    p_minus = (gamma_at_i(i)-1).*e_minus+(0.5).*rho_minus.*u_minus.^2;
    ...
    a_hat_minus = sqrt((gamma_at_i(i)-1)*(h_hat_minus-0.5*u_hat_minus.^2));    
...
%     title('X vs PRESSURE STEGER-WARMING EXPLICIT','FontSize',12);
